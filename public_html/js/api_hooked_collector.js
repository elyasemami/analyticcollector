/* api_hooked_collector.js  — CSE135
   Sends:
...
(() => {
  ...
  const pushQ = (ev) => { const q = loadQ(); q.push(ev); saveQ(q); };
  ...
  async function sendPerf() {
    ...
    const origin = performance.timeOrigin || (t ? t.navigationStart : Date.now());
    const relStart = nav?.startTime ?? 0;
    const relEnd   = (nav?.loadEventEnd ?? nav?.responseEnd ?? performance.now());

    const startedAt = nav ? origin + (relStart || 0) : (t?.navigationStart ?? null);
  ...
  // ---------- Activity ----------
  const addAct = (type, extra = {}) => pushQ({
    sessionId: SID, page: PAGE, ts: Date.now(), type, ...extra,
  });
  ...
  window.addEventListener('mousemove', (e) => {
    const t = Date.now(); if (t - lastMove < 120) return;
    lastMove = t; markActive();
  ...
  window.addEventListener('scroll', () => {
    const t = Date.now(); if (t - lastScroll < 200) return;
    lastScroll = t; markActive();
    addAct('scroll', { x: window.scrollX, y: window.scrollY });
  }, { passive: true });

  // Keyboard
  window.addEventListener('keydown', (e) => { markActive(); addAct('keydown', { key: e.key }); });
  window.addEventListener('keyup',   (e) => { markActive(); addAct('keyup',   { key: e.key }); });

  // Errors
  window.addEventListener('error', (e) => {
    addAct('error', { msg: e.message, src: e.filename, line: e.lineno, col: e.colno });
  });
  window.addEventListener('unhandledrejection', (e) => {
    addAct('error', { msg: (e.reason && (e.reason.message || String(e.reason))) || 'unhandledrejection' });
  });

  // Enter / Leave
  function pageEnter() { addAct('enter'); }
  function pageLeave() {
    addAct('leave');
  ...
  window.addEventListener('online', () => { flushQueue(); }, { passive: true });
